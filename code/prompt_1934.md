...
5. **Values in Thousands:** All balance sheet figures (liabilities and resources) are reported in thousands of dollars. Extract the numbers exactly as printed; do not multiply them.
...
* **statement_date:** Below the bank name, on the left side of the cell, there is a string indicating the date of the balance sheet statement, in the format `Mon'YYStmt`. For example:
    * "Dec'33Stmt" means December 1933 -> output `"1933-12"`
    * "Jul'33Stmt" means July 1933 -> output `"1933-07"`
    * "Mar'34Stmt" means March 1934 -> output `"1934-03"`
    * Always convert to YYYY-MM format. If unclear or absent, leave as `null`.
...
**Columns 7-12: Liabilities**
* Extract numerical values for the following six columns, which always appear in this order:
    1. `capital` (Capital)
    2. `surplus` (Surplus)
    3. `undivided_profits` (Undivided Profits and/or Reserves)
    4. `deposits` (Deposits)
    5. `other_liabs` (Other Liabilities)
    6. `totals` (Totals)
* **Preferred Capital:** The Capital column (column 7) may contain a second line with the annotation "Pf." followed by a number, indicating preferred stock capital (from RFC-era capital injections). For example:
    * "37 / Pf. 12" means common capital is 37 and preferred capital is 12.
    * "35 / Pf. 15" means common capital is 35 and preferred capital is 15.
    * Extract the main number as `capital` and the "Pf." number as `capital_preferred`.
    * If no "Pf." annotation is present, leave `capital_preferred` as `null`.
* **Surplus/Undivided Profits flags:** The undivided profits cell may contain special annotations instead of or alongside a number:
    * `surplus_consolidated`: Set to True if the cell says "(With Surp.)" or similar, indicating that surplus and undivided profits are combined in the surplus column. In this case, `undivided_profits` should be `null`.
    * `includes_reserves`: Set to True if the cell says "(Incl. Res.)" or similar, indicating the undivided profits value includes reserves.
    * Both flags default to False when no such annotation is present.
...
**Columns 13-17: Resources**
* Extract numerical values for the following five columns, which always appear in this order:
    1. `cash` (Cash, Exchanges & Due from Banks)
    2. `us_gov_securities` (U.S. Government Securities)
    3. `other_securities` (Other Securities)
    4. `loans` (Loans and Discounts)
    5. `other_resources` (Other Resources)
* Notice that column headers might change slightly across pages.
* For some pages, corresponding to smaller cities, not all five resource columns may be present. Fill in whichever columns are available and leave the rest as `null`.

**Column 18: Correspondents**
