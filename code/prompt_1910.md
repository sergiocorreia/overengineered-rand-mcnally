...
* **charter_number:** The charter number printed on a line under the bank name. Extract as a string (digits only; leave any surrounding punctuation out). Leave as `null` if unsure or absent.
...
* **Bank Events:** If a bank was closed, columns 3-6 (and sometimes even more columns) will contain a line with event information instead of the officer names name, and possibly the event date. Examples:
    ...
    * "(Merged with Bank XYZ, January 1, 1910)" -> `event`: "Merger with Bank XYZ", `event_date`: "1910-01-01"
    * "(Closed for liquidation, November 14, 1909)" -> `event`: "Closed for liquidation", `event_date`: "1909-11-14"
